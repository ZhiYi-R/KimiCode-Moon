...
- Only write under Kimi Code targets:
  - User-global: `$KIMI_CODE_HOME` if set, otherwise the platform app-data directory (e.g. %APPDATA%Kimi Code).
...
`$KIMI_CODE_HOME` before the platform app-data directory (e.g. %APPDATA%Kimi Code).
...
- `$KIMI_CODE_HOME/AGENTS.md`, or the platform app-data directory (e.g. %APPDATA%Kimi Code)/AGENTS.md if the env var is not
  set.
...
- `$KIMI_CODE_HOME/skills/`, or the platform app-data directory (e.g. %APPDATA%Kimi Code)/skills/ if the env var is not set.
...
- User-level source -> user-global MCP target (`$KIMI_CODE_HOME/mcp.json` or
  the platform app-data directory (e.g. %APPDATA%Kimi Code)/mcp.json`).
